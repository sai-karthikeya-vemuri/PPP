"""
Solving the Euler beam equation for a simple cantilever beam with load uniformly distributed.
EI y'' = M 
M = -p*X/2
x= 10m
...
def y(x,F,P):
    return ((-P*x**4)/(24*F)) + ((P*1000*x)/(6*F)) - ((P*10000/(8*F)))
...
plt.xlabel("X")
plt.ylabel("Y")
plt.title("Cantilever Beam with 10kN/m Uniformly distributed Load , E= 0.000005m^4,I =200000Mpa")
plt.legend()
plt.show()